...
from notes_api.schemas import UserUpdate

async def add_user(username: str, email: EmailStr, password: str | bytes, conn: Connection):
    sql = """INSERT INTO users(username, email, password) VALUES (%s, %s, %s) RETURNING *;"""
...
async def update_user(user_id: int, user: UserUpdate, conn: Connection):
