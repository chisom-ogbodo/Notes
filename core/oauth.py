...
from notes_api.schemas import Token
...
async def login_user(
    ...
    if not utils.verify_password(flow.password, user.get('password')):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid email or password')
    access_token = create_access_token({'sub': str(user.get('user_id'))})
